...
function checkMeshConnectivity(mesh: { _r_vertex: number[][], _triangles: Uint32Array, _halfedges: Int32Array }) {
    const { _r_vertex, _triangles, _halfedges } = mesh;
    // 1. make sure each side's opposite is back to itself
    // 2. make sure region-circulating starting from each side works
    let ghost_r = _r_vertex.length - 1, out_s: number[] = [];
...
function addGhostStructure(mesh: { _r_vertex: number[][], _triangles: Uint32Array, _halfedges: Int32Array }) {
    ...
    let r_unpaired_s: number[] = []; // seed to side
    ...
    let s_newstart_r = new Uint32Array(numSolidSides + 3 * numUnpairedSides);
...
class MeshBuilder {
    ...
    create(runChecks = false) {
        ...
        if (delaunator.triangles === undefined || delaunator.halfedges === undefined) {
            throw new Error("Delaunator failed to triangulate");
        }
        let graph: { _r_vertex: number[][], _triangles: Uint32Array, _halfedges: Int32Array, numSolidSides?: number, numBoundaryRegions?: number } = {
