 ...
 */
function find_coasts_t(mesh: TriangleMesh, r_ocean: boolean[]) {
    let coasts_t: number[] = [];
...
export function assign_t_elevation(
    t_elevation: number[], t_coastdistance: (number | null)[], t_downslope_s: number[],
    mesh: TriangleMesh,
    r_ocean: boolean[], r_water: boolean[], randInt: (N: number) => number
) {
    ...
    const t_ocean = (t: number) => r_ocean[mesh.s_begin_r(3 * t)];
    const r_lake = (r: number) => r_water[r] && !r_ocean[r];
    const s_lake = (s: number) => r_lake(mesh.s_begin_r(s)) || r_lake(mesh.s_end_r(s));
    ...
    while (queue_t.length > 0) {
        ...
        if (current_t === undefined) {
            throw new Error("current_t is undefined");
        }
        ...
        for (let i = 0; i < out_s.length; i++) {
            ...
            const cct = t_coastdistance[current_t];
            if (cct === null) {
                throw new Error("cct is null");
            }
            let newDistance = (lake ? 0 : 1) + cct;
            const cnt = t_coastdistance[current_t];
            if (cnt === null) {
                throw new Error("cnt is null");
            }
            if (t_coastdistance[neighbor_t] === null || newDistance < cnt) {
...
export function assign_r_elevation(r_elevation: number[], mesh: TriangleMesh, t_elevation: number[], r_ocean: boolean[]) {
...
export function redistribute_t_elevation(t_elevation: number[], mesh: TriangleMesh) {
