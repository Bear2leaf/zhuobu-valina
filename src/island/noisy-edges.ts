...
export function recursiveSubdivision(length: number, amplitude: number, randInt: (N: number) => number) {
    function recur(a: number[], b: number[], p: number[], q: number[]): number[][] {
...
export function assign_s_segments(s_lines: number[][][], mesh: TriangleMesh, options: {amplitude: number, length: number}, randInt: (N: number) => number) {
    const {amplitude, length} = options;
